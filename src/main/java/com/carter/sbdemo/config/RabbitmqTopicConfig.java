...
public class RabbitmqTopicConfig {
    ...
    @Bean
    public Binding bindingCoreExchange(Queue coreQueue, TopicExchange coreExchange) {//通配符“*”只能向后多匹配一层路径
        return BindingBuilder.bind(coreQueue).to(coreExchange).with("api.core.*");
    }

    @Bean
    public Binding bindingPaymentExchange(Queue paymentQueue, TopicExchange paymentExchange) {//#号可以向后匹配多层路径。
