...
public class RedisZSetUtil<K,V> {
     ...
     * @param:新增一个有序集合，存在的话为false，不存在的话为true
     * @return:
     */
    @Nullable
     ...
     * @param:返回有序集中指定成员的排名，其中有序集成员按分数值递增(从小到大)顺序排列
     * @return:
     */
    @Nullable
     ...
     * @param:返回有序集中指定成员的排名，其中有序集成员按分数值递减(从大到小)顺序排列
     * @return:
     */
    @Nullable
     ...
     * @param:通过索引区间返回有序集合成指定区间内的成员，其中有序集成员按分数值递增(从小到大)顺序排列
     * @return:
     */
    @Nullable
     ...
     * @param:通过索引区间返回有序集合成指定区间内的成员对象，其中有序集成员按分数值递增(从小到大)顺序排列
     * @return:
     */
    @Nullable
     ...
     * @param:通过分数返回有序集合指定区间内的成员，其中有序集成员按分数值递增(从小到大)顺序排列
     * @return:
     */
    @Nullable
     ...
     * @param:通过分数返回有序集合指定区间内的成员对象，其中有序集成员按分数值递增(从小到大)顺序排列
     * @return:
     */
    @Nullable
     ...
     * @param:通过分数返回有序集合指定区间内的成员，并在索引范围内，其中有序集成员按分数值递增(从小到大)顺序排列
     * @return:
     */
    @Nullable
     ...
     * @param:通过分数返回有序集合指定区间内的成员对象，并在索引范围内，其中有序集成员按分数值递增(从小到大)顺序排列
     * @return:
     */
    @Nullable
     ...
     * Parameters:
     * @param:获取有序集合的成员数，内部调用的就是zCard方法
     * @return:
     ...
     * @param:移除指定索引位置的成员，其中有序集成员按分数值递增(从小到大)顺序排列
     * @return:
     */
    @Nullable
